...
module.exports = async (req, res) => {
  ...
  if (isEnabled()) {
    try {
      ...
      const { data, error } = await supabase.from('quiz_entries').select('*').order('id', { ascending: true });
      if (error) {
        res.statusCode = 500;
  ...
  const items = list.map((item, index) => ({ id: index + 1, name: item.name, answer: item.answer, at: item.at }));
  res.setHeader('Content-Type', 'application/json');
  return res.end(JSON.stringify({ items }));
};
